...
// AI quotes now use a local server endpoint backed by OpenAI.
// On GitHub Pages, this will be disabled automatically.
const AI_ENABLED = !String(location.hostname).endsWith('github.io');
    ...
    if (quoteElement) {
        ...
        try {
            ...
            const resp = await fetch('/api/ai-quote', {
                ...
                body: JSON.stringify({ weather: weatherMain, temperature: weatherData.main?.temp })
            });
            const data = await resp.json();
            if (!resp.ok || !data?.quote) {
                throw new Error(data?.error || 'AI quote not available');
            }
            quoteElement.textContent = data.quote || fallbackQuote;
        } catch (e) {
            console.warn('AI quote failed; using fallback quote.', e);
            quoteElement.textContent = fallbackQuote;
        }
    }
